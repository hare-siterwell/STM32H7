...
void motor_divnum(struct Motor *motor, u16 divnum) {
  ...
  HAL_GPIO_WritePin(motor->ms1_gpio_array, motor->ms1_gpio, ms1);
  HAL_GPIO_WritePin(motor->ms2_gpio_array, motor->ms2_gpio, ms2);
  HAL_GPIO_WritePin(motor->ms3_gpio_array, motor->ms3_gpio, ms3);
}
...
int motor_move(struct Motor *motor, u16 divnum, s32 step_move, u16 step_spmax,
  ...
  if (!step_move || !step_spmax || !step_accel) {
    ...
    HAL_TIM_Base_Stop_IT(motor1.htim);
    return 1;
  }

  if (step_move < 0) { // 步数为负,反转
    HAL_GPIO_WritePin(motor->dir_gpio_array, motor->dir_gpio, GPIO_PIN_RESET);
    step_move = -step_move;
  } else {
    HAL_GPIO_WritePin(motor->dir_gpio_array, motor->dir_gpio, GPIO_PIN_SET);
  }
  ...
  return HAL_TIM_Base_Start_IT(motor1.htim);
}
...
void motor_spta_algorithm(struct Motor *motor) {
  ...
  // 拉低脉冲信号
  HAL_GPIO_WritePin(motor->step_gpio_array, motor->step_gpio, GPIO_PIN_RESET);
  ...
  if (carry) { // 溢出
    ...
    motor->step_count++;
    // 拉高脉冲信号产生一个步进脉冲
    HAL_GPIO_WritePin(motor->step_gpio_array, motor->step_gpio, GPIO_PIN_SET);
  }
